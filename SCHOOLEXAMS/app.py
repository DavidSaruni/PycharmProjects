...
def search():
    if request.method == "POST":
        ...
        if data:
            ...
            return render_template("calculator.html", msg='No student found! Try register the student')
    else:
        return render_template('calculator.html')
...
def report():
    ...
    sql_query = "SELECT * FROM calculation_table, registration_table"
    cursor.execute(sql_query)
    if cursor.rowcount < 1:
        return render_template('report.html', msg ='no student found, record is empty')
    else:
        #now we can get all the rows returned by the cursor
        rows = cursor.fetchall()
        #after getting the rows we need
        #we send of all of the row to the html page
...
def search2():
    if request.method == "POST":
        ...
        cursor.execute("SELECT * from calculation_table WHERE admission = %s", admission)
        conn.commit()
        data = cursor.fetchall()
        ...
        if data:
            ...
            msg = "student not found"
            return render_template("calculator.html", msg = "student not found")
    else:
...
def results():
    if 'username' in session:
        ...
        sql_query = "SELECT * FROM calculation_table"
        cursor.execute(sql_query)
        if cursor.rowcount < 1:
...
#         cursor = con.cursor()
#         sql = "delete from calculation_table where username = (%s)"
#
#         try:
#             # when connection is established successfully
