...
def registration():
    if request.method == 'POST':
        ...
        conn = pymysql.connect("localhost", 'root', '', 'real_estate')
        cursor = conn.cursor()

        insert_query = "INSERT INTO `registration_table` (`id_number`, `username`, `password`, `email`, `location`, `phone`) VALUES  (%s,%s,%s,%s,%s,%s)"
        if insert_query:
...
def login():
    ...
    if request.method == 'POST':
        ...
        if cursor.rowcount ==0:
            ...
            return render_template('login.html', acc_error_msg = 'Too many accounts detected')
        else:
...
def check_out():
    if request.method == 'POST':
        ...
        con = pymysql.connect("localhost", "root", "", "real_estate")
        cursor = con.cursor()
...
def makeConnection():
  return pymysql.connect("localhost", "root", "", "real_estate", )
